...
class Promotion extends CI_Controller {
	...
	function index(){
		...
		render('apps/promotion/index',$data,'apps');
	}
	
	public function add($id=''){
		if($id){
			// $data = $this->promotionmodel->findById($id);
			$datas 	= $this->promotionmodel->selectData($id);
		...
		render('apps/promotion/add',$data,'apps');
	}
	public function view($id=''){
		...
		render('apps/promotion/view',$data,'blank');
	}
	function records(){
		$data = $this->promotionmodel->records();
		...
		render('apps/promotion/records',$data,'blank');
	}
		...
		foreach ($post['title'] as $key => $value){
			...
			if ($this->form_validation->run() == FALSE){
				...
				if($idedit && $post['img'][$key]){
					...
					$datas 				= $this->promotionmodel->selectData($idedit);
				...
				if($idedit){
					if($key==0){
						...
						$iddata 		= $this->promotionmodel->update($data_save,$idedit);
					}else{
						auth_update();
						$ret['message'] = 'Update Success';
						$act			= "Update promotion";
						$iddata 		= $this->promotionmodel->updateKedua($data_save,$idedit);
					}
				}else{
					auth_insert();
					$ret['message'] = 'Insert Success';
					$act			= "Insert promotion";
					$iddata 		= $this->promotionmodel->insert($data_save);
				}
	...
	function del(){
		...
		$this->promotionmodel->delete2($id);
		$this->db->trans_complete();
	}
...
/* Location: ./application/controllers/apps/promotion.php */