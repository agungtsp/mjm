...
class Choose extends CI_Controller {
	...
	function index(){
		...
		render('apps/choose/index',$data,'apps');
	}
	
	public function add($id=''){
		if($id){
			// $data = $this->choosemodel->findById($id);
			$datas 	= $this->choosemodel->selectData($id);
		...
		render('apps/choose/add',$data,'apps');
	}
	public function view($id=''){
		...
		render('apps/choose/view',$data,'blank');
	}
	function records(){
		$data = $this->choosemodel->records();
		...
		render('apps/choose/records',$data,'blank');
	}
		...
		foreach ($post['title'] as $key => $value){
			...
			if ($this->form_validation->run() == FALSE){
				...
				if($idedit && $post['img'][$key]){
					...
					$datas 				= $this->choosemodel->selectData($idedit);
				...
				if($idedit){
					if($key==0){
						...
						$act			= "Update choose";
						$iddata 		= $this->choosemodel->update($data_save,$idedit);
					}else{
						auth_update();
						$ret['message'] = 'Update Success';
						$act			= "Update choose";
						$iddata 		= $this->choosemodel->updateKedua($data_save,$idedit);
					}
				}else{
					auth_insert();
					$ret['message'] = 'Insert Success';
					$act			= "Insert choose";
					$iddata 		= $this->choosemodel->insert($data_save);
				}
	...
	function del(){
		...
		$this->choosemodel->delete($id);
		$this->choosemodel->delete2($id);
		$this->db->trans_complete();
	}
...
/* Location: ./application/controllers/apps/choose.php */