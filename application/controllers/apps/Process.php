...
class Process extends CI_Controller {
	...
	function index(){
		...
		render('apps/process/index',$data,'apps');
	}
	
	public function add($id=''){
		if($id){
			// $data = $this->processmodel->findById($id);
			$datas 	= $this->processmodel->selectData($id);
		...
		render('apps/process/add',$data,'apps');
	}
	public function view($id=''){
		...
		render('apps/process/view',$data,'blank');
	}
	function records(){
		$data = $this->processmodel->records();
		...
		render('apps/process/records',$data,'blank');
	}
			...
			if ($this->form_validation->run() == FALSE){
				...
				if($idedit){
					if($key==0){
						...
						$act			= "Update process";
						$iddata 		= $this->processmodel->update($data_save,$idedit);
					}else{
						auth_update();
						$ret['message'] = 'Update Success';
						$act			= "Update process";
						$iddata 		= $this->processmodel->updateKedua($data_save,$idedit);
					}
				}else{
					auth_insert();
					$ret['message'] = 'Insert Success';
					$act			= "Insert process";
					$iddata 		= $this->processmodel->insert($data_save);
				}
...
/* Location: ./application/controllers/apps/process.php */