...
class Testimonial extends CI_Controller {
	...
	function index(){
		...
		render('apps/testimonial/index',$data,'apps');
	}
	
	public function add($id=''){
		if($id){
			// $data = $this->testimonialmodel->findById($id);
			$datas 	= $this->testimonialmodel->selectData($id);
		...
		render('apps/testimonial/add',$data,'apps');
	}
	public function view($id=''){
		...
		render('apps/testimonial/view',$data,'blank');
	}
	function records(){
		$data = $this->testimonialmodel->records();
		...
		render('apps/testimonial/records',$data,'blank');
	}
		...
		foreach ($post['title'] as $key => $value){
			...
			if ($this->form_validation->run() == FALSE){
				...
				if($idedit && $post['img'][$key]){
					...
					$datas 				= $this->testimonialmodel->selectData($idedit);
				...
				if($idedit){
					if($key==0){
						...
						$act			= "Update testimonial";
						$iddata 		= $this->testimonialmodel->update($data_save,$idedit);
					}else{
						auth_update();
						$ret['message'] = 'Update Success';
						$act			= "Update testimonial";
						$iddata 		= $this->testimonialmodel->updateKedua($data_save,$idedit);
					}
				}else{
					auth_insert();
					$ret['message'] = 'Insert Success';
					$act			= "Insert testimonial";
					$iddata 		= $this->testimonialmodel->insert($data_save);
				}
	...
	function del(){
		...
		$this->testimonialmodel->delete2($id);
		$this->db->trans_complete();
	}
...
/* Location: ./application/controllers/apps/testimonial.php */