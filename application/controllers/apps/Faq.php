...
class Faq extends CI_Controller {
	...
	function index(){
		...
		render('apps/faq/index',$data,'apps');
	}
	
	public function add($id=''){
		if($id){
			// $data = $this->faqmodel->findById($id);
			$datas 	= $this->faqmodel->selectData($id);
	...
	public function view($id=''){
		...
		render('apps/faq/view',$data,'blank');
	}
	function records(){
		$data = $this->faqmodel->records();
		...
		render('apps/faq/records',$data,'blank');
	}
		...
		foreach ($post['title'] as $key => $value){
			...
			if ($this->form_validation->run() == FALSE){
				...
				if($idedit && $post['img'][$key]){
					...
					$datas 				= $this->faqmodel->selectData($idedit);
				...
				if($idedit){
					if($key==0){
						...
						$act			= "Update faq";
						$iddata 		= $this->faqmodel->update($data_save,$idedit);
					}else{
						auth_update();
						$ret['message'] = 'Update Success';
						$act			= "Update faq";
						$iddata 		= $this->faqmodel->updateKedua($data_save,$idedit);
					}
				}else{
					auth_insert();
					$ret['message'] = 'Insert Success';
					$act			= "Insert faq";
					$iddata 		= $this->faqmodel->insert($data_save);
				}
...
/* End of file faq.php */
/* Location: ./application/controllers/apps/faq.php */